...
이 문서는 `market-risk-var-backtesting` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 시장 리스크 VaR 백테스팅 (Market Risk VaR Backtesting)
- 연구 카테고리: Stochastic Models & Market Risk
- 핵심 질문: 과거 손익 분포로 추정한 VaR는 실제 손실 예외를 어느 정도 설명하는가?
...
- KAIST-DFMBA/Management-Statistics-Analysis/
- KAIST-DFMBA/Quant-and-Factor-Investment-Strategies/

## 참고 초점

- 수익률 분포, 분위수, tail risk 계산
- 표본 기반 위험지표를 검증 지표와 연결하는 방식
- 예외 발생률과 Expected Shortfall 해석
